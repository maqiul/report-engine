...
| 项目 | 覆盖范围 | 测试数 |
|------|----------|--------|
| `ReportEngine.Core.Tests` | 模板解析 / 表达式引擎 / ZXing 条码 / 子报表解析（FileSystem + Composite）/ ClosedXML 导出 / 渲染主路径 + ReportRenderer 边界（ReportHeader / PageHeader / SubBands / 多栏 / 极小页面） | **62** |
...
当前版本：**v0.1.12**（33 commits / 12 tags；详细里程碑见 `git tag -l`）
...
| 0.5.x | WinForms 端对齐 | ⏳ |
| 1.0   | 文档、单元测试、首个稳定 NuGet 发布 | 🚧（已 72 个测试） |
