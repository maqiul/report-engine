...
XAML 设计器自动生成的代码 + 业务逻辑全堆在一起。v0.1.6 已按职责拆出 15 个独立文件：
...
**累计**：**-1451 行 / -28.7%**。剩余 3599 行主要是 WPF 控件事件处理（拖拽/选中/缩放/快捷键）、
属性面板的 8 个 `AddProp*` 工厂方法（line 2675-2850 ~175 行，与 selectedElement 状态耦合），
与 DispatcherTimer / 剪贴板等运行时状态深度耦合，ROI 较低，暂留至 v0.4.x 重构。
候选续拆：Step4.A 抽属性面板的 8 个 `AddProp*` 工厂（~175 行）/ Step4.B 抽 `LoadPreviewData` + `ShowShortcutsDialog` 等杂项。
