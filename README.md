...
| 项目 | 覆盖范围 | 测试数 |
|------|----------|--------|
| `ReportEngine.Core.Tests` | 模板解析 / 表达式引擎 / ZXing 条码 / 子报表解析（FileSystem + Composite）/ ClosedXML 导出 / 渲染主路径 | **57** |
...
当前版本：**v0.1.11**（32 commits / 11 tags；详细里程碑见 `git tag -l`）
...
| 0.5.x | WinForms 端对齐 | ⏳ |
| 1.0   | 文档、单元测试、首个稳定 NuGet 发布 | 🚧（已 67 个测试） |
