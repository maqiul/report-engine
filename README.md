...
当前版本：**v0.1.20**（42 commits / 21 tags；详细里程碑见 `git tag -l`）
...
**v0.2 目标**：把 `MainWindow.cs` 进一步拆到 < 3000 行；把测试覆盖扩到 100 条；把文档化补齐。 **✅ 全部达成！**

#### 已识别候选（按 ROI 排序）— 全部完成

| 优先级 | 任务 | 预估影响 | 状态 / tag |
|--------|------|----------|------------|
| 🥇 P0 | **Step4.A** 抽 8 个 `AddProp*` 工厂方法 | MainWindow -175 行 | ✅ v0.1.9 |
| 🥇 P0 | **D2** 补 ClosedXmlExporter 边界用例 | +6 测试 | ✅ v0.1.10 |
| 🥈 P1 | **A1** 补 SubReport/Barcode 集成测试 | +4 测试 | ✅ v0.1.11 |
| 🥈 P1 | **Step4.B** 抽 `LoadPreviewData` + `ShowShortcutsDialog` + 杂项弹窗 | MainWindow -120 行 | ✅ v0.1.14~v0.1.17 |
| 🥉 P2 | **D3** 补 `ReportRenderer` 边界用例 | +5 测试 | ✅ v0.1.12 |
| 🥉 P2 | **D4** 补 `ExpressionEngine` 边界用例 | +4 测试 | ✅ v0.1.13 |
| ⏳ P3 | **D5~D8** 补 Designer/PdfSharp/ClosedXml/Barcode 边界用例 | +20 测试 | ✅ v0.1.18 |
| ⏳ P3 | **代码规范 + CI** | `.editorconfig` + GitHub Actions | ✅ v0.1.20 |
...
```bash
# 当前：100/100 全绿（net8.0）
dotnet test tests/ReportEngine.Core.Tests/ReportEngine.Core.Tests.csproj
```

测试只针对跨平台 `ReportEngine.Core` 模块（解析、表达式、子报表、条码、Excel/PDF 导出、渲染路径）。
...
**v0.2 路线图已全部达成**（详见上节"🗺️ v0.2 详细路线图"）。

**下一步候选**（按 ROI 排序）：

1. **P1** 继续拆 `MainWindow.cs` 奔 2500 行（事件处理 / 拖拽 / 选中 是下一块大头）
2. **P1** 补 WinForms / Designer.Wpf 测试项目（需 net8.0-windows 测试 target）
3. **P2** 加 `CHANGELOG.md` + 正式发布 v0.2.0（去 `-alpha` 后缀）
4. **P2** NuGet 发布到 nuget.org（需 API key）
5. **P3** `ReportRenderer` 性能优化（大模板 / 多页）
