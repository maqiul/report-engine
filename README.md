...
│   ├── MainWindow.cs              ← 4109 行（已从 5050 拆出 941 行）
...
| 0.4.x | WPF 可视化设计器 | 🚧（MainWindow 已从 5050 → 4109 行） |
| 0.5.x | WinForms 端对齐 | ⏳ |
...
XAML 设计器自动生成的代码 + 业务逻辑全堆在一起。v0.1.2 已按职责拆出 12 个独立文件：
...
**累计**：**-941 行 / -18.6%**。剩余 4109 行主要是 WPF 控件事件处理（拖拽/选中/缩放/快捷键），
与 DispatcherTimer / 剪贴板等运行时状态深度耦合，ROI 较低，暂留至 v0.4.x 重构。
候选续拆：Step3.C 抽 `ShowPageSetupDialog` 页面设置弹窗（~150 行）。
