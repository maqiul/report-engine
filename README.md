...
│   ├── MainWindow.cs              ← 3846 行（已从 5050 拆出 1204 行）
...
| 0.4.x | WPF 可视化设计器 | 🚧（MainWindow 已从 5050 → 3846 行） |
| 0.5.x | WinForms 端对齐 | ⏳ |
...
XAML 设计器自动生成的代码 + 业务逻辑全堆在一起。v0.1.4 已按职责拆出 13 个独立文件：
...
| Step 3.C | 页面设置弹窗 | `PageSetupDialog.cs` | 4109 → 3846（-263） |

**累计**：**-1204 行 / -23.8%**。剩余 3846 行主要是 WPF 控件事件处理（拖拽/选中/缩放/快捷键），
与 DispatcherTimer / 剪贴板等运行时状态深度耦合，ROI 较低，暂留至 v0.4.x 重构。
候选续拆：Step3.D 抽 `ShowFontDialog` 字体选择弹窗（~150 行）/ `ShowExpressionEditor` 表达式编辑器（~70 行）。
